...
Extract these important numbers, one item each: grade weights (e.g. a component worth 25%), grading-scale thresholds (e.g. A = 93), curve or grade-adjustment targets (e.g. "the median is shifted to 81%"), assessment counts (e.g. 3 exams), late or attendance penalties (e.g. 10% per day), credit hours or units, due dates and exam dates, class or office-hours times, and section numbers. The full grading policy matters: if the syllabus states more than one grading method, a curve/adjustment rule, or a full letter-grade cut-off table, extract every stated number in it, not just the headline weights. Do not extract page numbers, footnote markers, or numbers with no course meaning.

Copy "value" verbatim from the source line — the exact number token exactly as written, including its unit or symbol: "25%", "3", "March 12", "10:00 AM". Never convert, round, or reformat it. Every item "source_quote" must be one exact line from the transcription data — except a grading-scale table printed as one line of letter grades immediately followed by one line of their thresholds (nothing else between them), where "source_quote" may be those two exact lines joined by a single "\n", label line first. Prefer the single-line form whenever the label and value already share a line. "raw_label" must be a verbatim label substring of its own source_quote and must contain letters, never be the number itself. Choose the value that is the nearest number of its kind to its label on that line. Set "category" to the best fit and "kind" to the value's shape; both are re-derived downstream, so consistency matters more than perfect choice. Copy "transcription" byte-for-byte from the source data; do not clean it up.
